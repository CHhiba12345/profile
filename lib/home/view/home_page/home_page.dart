...
class _HomeControllerState extends State<HomePage> {
  ...
  Future<void> _addNewTask(String task, String employeeId) async {
    ...
    if (employeeIdInTask != null &&
        employeeIdInTask is String &&
        employeeIdInTask == employeeId) {
  ...
  void _showTaskInputDialog(BuildContext context, Employee employee) {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text(
              'Ajouter une Tâche pour ${employee.firstName} ${employee.lastName}'),
          content: Column(
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) =>
                        ProfilePage(userController: _userController),
                  ),
                );
              },
  ...
  void _showTasksListDialog(BuildContext context, Employee employee) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text(
              'Liste des Tâches de ${employee.firstName} ${employee.lastName}'),
          content: Container(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                return ListTile(
                  ...
                  subtitle: getTask(employee.tasks)[index].completed
                      ? Text('Terminé')
                      : Text('En cours'),
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      if (!getTask(employee.tasks)[index].completed &&
                          widget.email != 'chaoualihiba25@gmail.com')
                        ElevatedButton(
                          onPressed: () {
                            _markTaskAsCompleted(
                                getTask(employee.tasks)[index].id);
                            _showCompletionMessage(
                                context, getTask(employee.tasks)[index].id);
                          },
                          child: Text('Terminer'),
                        ),
                      if (widget.email == 'chaoualihiba25@gmail.com')
                        IconButton(
                          ...
                          onPressed: () {
                            _showEditTaskDialog(
                                context, getTask(employee.tasks)[index]);
                          },
                        ),
                      if (widget.email == 'chaoualihiba25@gmail.com')
                        IconButton(
